...
// No need to import 'ColorBalance', 'HistoryState', 'ToneCurvesState', 'ToneCurvePoint' as they are for type checking.
...
export const applyBackgroundColor = (ctx, color) => {
  ctx.save();
...
export const applyGradientFilter = (ctx, startColor, endColor, intensity) => {
...
export const applyContourFilter = (ctx, intensity) => {
    if (intensity === 0) return;
...
export const applyVignette = (ctx, intensity, size) => {
  if (intensity === 0) return;
...
export const applyAllPixelAdjustments = (ctx, state) => {
    const { 
        contrast,
    ...
    const isDefaultCurves = (curves) => {
      const isDefaultPoints = (points, hasMidpoint) => {
        if (hasMidpoint) {
