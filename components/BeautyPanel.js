...
const BeautySlider = ({ label, value, min = -100, max = 100, onChange, onCommit, disabled }) => {
  ...
  return (
    React.createElement("div", { className: "flex flex-col gap-1.5 text-sm" },
      React.createElement("div", { className: "flex justify-between items-center" },
        React.createElement("label", { className: "font-medium text-slate-300 uppercase tracking-wider text-xs" }, label),
        React.createElement("span", { className: "text-xs font-mono bg-slate-800/80 text-slate-200 px-2 py-1 rounded-md w-12 text-center" },
          value
        )
      ),
...
export const BeautyPanel = ({ onAiEdit, isLoading, selectedTool }) => {
  const [values, setValues] = useState(
    adjustments.reduce((acc, adj) => ({ ...acc, [adj.feature]: 0 }), {})
  );
  const [hairColorDescription, setHairColorDescription] = useState('');
  ...
  return (
    React.createElement("div", { className: "flex flex-col gap-4" },
      ...
      React.createElement("hr", { className: "border-slate-700/50" }),
      
      adjustments.map(adj => (
        React.createElement(BeautySlider, {
          key: adj.feature,
          label: adj.label,
      ...
      React.createElement("div", { className: "flex flex-col gap-1.5 text-sm" },
        React.createElement("label", { htmlFor: "hair-color-description", className: "font-medium text-slate-300 uppercase tracking-wider text-xs" }, "Cor do Cabelo"),
