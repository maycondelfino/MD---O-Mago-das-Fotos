...
// Garantimos que process esteja definido para evitar crashes.
const API_KEY = (typeof process !== 'undefined' && process.env) ? process.env.API_KEY : '';
const ai = new GoogleGenAI({ apiKey: API_KEY });

// Helper function to handle AI errors gracefully
const handleAiError = (error, signal) => {
  if (signal?.aborted) {
...
const generateAiErrorFromResponse = (response) => {
...
export const editImage = async ({
  ...
  try {
    ...
    // The order is important to match the prompt engineering in App.js
    if (referenceImage) {
...
export const generateImage = async ({ prompt, signal, model }) => {
    try {
        const response = await ai.models.generateContent({
            model: model, // Utilizando o modelo Gemini 3 Pro
...
export const analyzeImage = async ({ base64ImageData, mimeType, signal, model }) => {
    try {
